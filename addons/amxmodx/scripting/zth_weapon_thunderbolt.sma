...
#define PLUGIN "[Mileage] Primary: Thunderbolt"
#define VERSION "2014"
#define AUTHOR "Dias"
...
new g_Thunderbolt
new g_Had_Thunderbolt, g_Zoom[33], g_OldWeapon[33]
new g_IsAlive
new Float:CheckDelay[33], g_TempAttack
...
public plugin_init() 
{
	...
	g_Thunderbolt = zp_register_extra_item("Thunderbolt", 0, ZP_TEAM_HUMAN)
}
...
{
	register_native("Get_Thunderbolt", "Mileage_WeaponGet", 1)
	register_native("Refill_Thunderbolt", "Mileage_WeaponRefillAmmo", 1)
}
...
public Event_CurWeapon(id)
{
	...
	if((CSWID == CSW_THUNDERBOLT && g_OldWeapon[id] != CSW_THUNDERBOLT) && Get_BitVar(g_Had_Thunderbolt, id))
	{
		if(SubModel != -1) Draw_NewWeapon(id, CSWID)
		UpdateAmmo(id, -1, cs_get_user_bpammo(id, CSW_THUNDERBOLT))
	} else if((CSWID == CSW_THUNDERBOLT && g_OldWeapon[id] == CSW_THUNDERBOLT) && Get_BitVar(g_Had_Thunderbolt, id)) {
		...
		UpdateAmmo(id, -1, cs_get_user_bpammo(id, CSW_THUNDERBOLT))
	} else if(CSWID != CSW_THUNDERBOLT && g_OldWeapon[id] == CSW_THUNDERBOLT) {
		if(SubModel != -1) Draw_NewWeapon(id, CSWID)
	}
...
{
	if(CSW_ID == CSW_THUNDERBOLT)
	{
		static ent
		ent = fm_get_user_weapon_entity(id, CSW_THUNDERBOLT)
		
		if(pev_valid(ent) && Get_BitVar(g_Had_Thunderbolt, id))
		{
...
public Set_SniperModel(id)
{
	...
	cs_set_user_zoom(id, g_Zoom[id], 1)
}
