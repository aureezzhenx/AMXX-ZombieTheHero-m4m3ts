/*================================================================================
	...
	*********************************************************************
	************** [Zombie The Hero "m4m3ts" NextGen 2017] **************
	*********************************************************************
	
	.SMA ini sudah dibedah-bedah agar kompatibel dengan perubahan sebagai
	berikut:
	
	- Penghapusan beberapa Array pada Resources 	
	(bisa di cek di mamet_zombiethehero.ini)
	- Penghapusan HUD 
	- Hurt Zombie
	
	NextGen by aureezz
	
	Diharap jangan di edit-edit lagi.. gue udah cape
...
new const ZP_CUSTOMIZATION_FILE[] = "mamet_zombiethehero.ini"
new const ZP_EXTRAITEMS_FILE[] = "mamet_zp_extraitems.ini"
new const ZP_ZOMBIECLASSES_FILE[] = "mamet_zp_zombieclasses.ini"
...
// Plugin Version
new const PLUGIN_VERSION[] = "m4m3ts2017aureezz"
...
new const sound_zombie_male_hitfall[] = "zombie_plague/regular_hurt.wav"
new const sound_zombie_male_die[] = "zombie_plague/regular_death.wav"

new const sound_zombie_female_hitfall[] = "zombie_plague/light_hurt.wav"
new const sound_zombie_female_die[] = "zombie_plague/light_death.wav"

new const sound_zombie_stamper_hitfall[] = "zombie_plague/stamper_hurt.wav"
new const sound_zombie_stamper_die[] = "zombie_plague/stamper_death.wav"

new const sound_zombie_sting_hitfall[] = "zombie_plague/resident_hurt1.wav"
new const sound_zombie_sting_die[] = "zombie_plague/resident_death.wav"

new const sound_zombie_banshee_hitfall[] = "zombie_plague/zombi_banshee_hurt.wav"
new const sound_zombie_banshee_die[] = "zombie_plague/zombi_banshee_death.wav"

new const sound_zombie_venom_hitfall[][] = {"zombie_plague/boomer_hurt1.wav", "zombie_plague/boomer_hurt2.wav"}

new const sound_zombie_meta_hitfall[] = "zombie_plague/metatronic_pain_hurt.wav"
new const sound_zombie_lilith_hitfall[] = "zombie_plague/lilith_pain_hurt.wav"

new const sound_nvg[2][] = {"items/nvg_off.wav", "items/nvg_on.wav"}
new const infectionmodel[] = "models/zombie_plague/v_infection.mdl"
...
model_vknife_admin_human[64], model_vknife_admin_zombie[64], Array:sound_win_zombies, Array:sound_win_zombies_ismp3,
Array:sound_win_humans, Array:sound_win_humans_ismp3, Array:zombie_miss_wall, 
Array:zombie_hit_normal, Array:zombie_hit_stab, g_ambience_rain, g_ambience_sounds[MAX_AMBIENCE_SOUNDS],
...
Array:sound_ambience5_duration, Array:g_primary_items, Array:g_secondary_items, Array:g_additional_items,
...
cvar_multiratio, cvar_flaresize, cvar_spawndelay,cvar_extraweapons, 
cvar_extranvision, cvar_nvggive, cvar_preventconsecutive, cvar_botquota,
...
cvar_plaguenemnum, cvar_plaguenemhpmulti, cvar_plaguesurvhpmulti,
cvar_survweapon, cvar_plaguesurvnum, cvar_infectionscreenfade, cvar_infectionscreenshake,
cvar_infectionsparkle, cvar_infectiontracers, cvar_infectionparticles, cvar_allowrespawnsurv, 
cvar_flashshowall, cvar_allowrespawninfection, cvar_allowrespawnnem,
...
cvar_startammopacks, cvar_randweapons,cvar_adminknifemodelshuman, cvar_adminknifemodelszombie, 
cvar_keephealthondisconnect,cvar_buyzonetime, cvar_huddisplay
...
public plugin_precache()
{
	...
	register_plugin("Zombie The Hero m4m3ts NextGen", PLUGIN_VERSION, "aureezz")
	...
	for(i = 0; i < sizeof(sound_zombie_venom_hitfall); i++)
	precache_sound(sound_zombie_venom_hitfall[i])
	
	precache_sound(sound_zombie_male_hitfall)
	precache_sound(sound_zombie_male_die)
	precache_sound(sound_zombie_female_hitfall)
	precache_sound(sound_zombie_female_die)
	precache_sound(sound_zombie_stamper_hitfall)
	precache_sound(sound_zombie_stamper_die)
	precache_sound(sound_zombie_banshee_hitfall)
	precache_sound(sound_zombie_banshee_die)
	precache_sound(sound_zombie_sting_hitfall)
	precache_sound(sound_zombie_sting_die)
	precache_sound(sound_zombie_meta_hitfall)
	precache_sound(sound_zombie_lilith_hitfall)
	precache_model(infectionmodel)
...
public plugin_init()
{
	...
	// Language files
	register_dictionary("mamet_zombiethehero.txt")
	...
	// Format mod name
	formatex(g_modname, charsmax(g_modname), "Zombie The Hero m4m3ts NextGen %s", PLUGIN_VERSION)
...
public plugin_cfg()
{
	...
	server_cmd("exec %s/mamet_zombiethehero.cfg", cfgdir)
...
public logevent_round_end()
{
	...
	if (!fnGetZombies())
	{
		...
		format(iText, charsmax(iText), "Human Win!")
	...
	else if (!fnGetHumans())
	{
		...
		format(iText, charsmax(iText), "Zombie Win!")
		zp_clientcenter_text(0, iText)
...
public remove_nvsions(id) SwitchNvg(id, 0)
...
public fw_PlayerSpawn_Post(id)
{
	...
	else set_task(0.5, "remove_nvsions", id)
	...
	if (get_pcvar_num(cvar_buycustom))
		set_task(0.0, "show_menu_mamet", id+TASK_SPAWN) 
...
public fw_EmitSound(id, channel, const sample[], Float:volume, Float:attn, flags, pitch)
{
	...
	{
		if(zth_get_zombie_class(id) == 0)
		{
			emit_sound(id, CHAN_VOICE, sound_zombie_sting_hitfall, 1.0, ATTN_NORM, 0, PITCH_NORM)
		}
		if(zth_get_zombie_class(id) == 1)
		{
			emit_sound(id, CHAN_VOICE, sound_zombie_male_hitfall, 1.0, ATTN_NORM, 0, PITCH_NORM)
		}
		if(zth_get_zombie_class(id) == 2)
		{
			emit_sound(id, CHAN_VOICE, sound_zombie_female_hitfall, 1.0, ATTN_NORM, 0, PITCH_NORM)
		}
		if(zth_get_zombie_class(id) == 3)
		{
			emit_sound(id, CHAN_VOICE, sound_zombie_stamper_hitfall, 1.0, ATTN_NORM, 0, PITCH_NORM)
		}
		if(zth_get_zombie_class(id) == 4)
		{
			emit_sound(id, CHAN_VOICE, sound_zombie_banshee_hitfall, 1.0, ATTN_NORM, 0, PITCH_NORM)
		}
		if(zth_get_zombie_class(id) == 5)
		{
			emit_sound(id, CHAN_VOICE, sound_zombie_venom_hitfall[random( sizeof(sound_zombie_venom_hitfall))], 1.0, ATTN_NORM, 0, PITCH_NORM)
		}
		if(zth_get_zombie_class(id) == 6)
		{
			emit_sound(id, CHAN_VOICE, sound_zombie_meta_hitfall, 1.0, ATTN_NORM, 0, PITCH_NORM)
		}
		if(zth_get_zombie_class(id) == 7)
		{
			emit_sound(id, CHAN_VOICE, sound_zombie_lilith_hitfall, 1.0, ATTN_NORM, 0, PITCH_NORM)
		}
		return FMRES_SUPERCEDE;
	}
	...
	if (sample[7] == 'd' && ((sample[8] == 'i' && sample[9] == 'e') || (sample[8] == 'e' && sample[9] == 'a')))
	{
		if(zth_get_zombie_class(id) == 0)
		{
			emit_sound(id, CHAN_VOICE, sound_zombie_sting_die, 1.0, ATTN_NORM, 0, PITCH_NORM)
		}
		if(zth_get_zombie_class(id) == 1)
		{
			emit_sound(id, CHAN_VOICE, sound_zombie_male_die, 1.0, ATTN_NORM, 0, PITCH_NORM)
		}
		if(zth_get_zombie_class(id) == 2)
		{
			emit_sound(id, CHAN_VOICE, sound_zombie_female_die, 1.0, ATTN_NORM, 0, PITCH_NORM)
		}
		if(zth_get_zombie_class(id) == 3)
		{
			emit_sound(id, CHAN_VOICE, sound_zombie_stamper_die, 1.0, ATTN_NORM, 0, PITCH_NORM)
		}
		if(zth_get_zombie_class(id) == 4)
		{
			emit_sound(id, CHAN_VOICE, sound_zombie_banshee_die, 1.0, ATTN_NORM, 0, PITCH_NORM)
		}
		return FMRES_SUPERCEDE;
	}
	...
	if (g_nvision[id] || (g_isalive[id] && cs_get_user_nvg(id)))
	{
		...
		if (get_pcvar_num(cvar_customnvg))
		{
			if (!g_nvg[id])
				{
					SwitchNvg(id, 1)
					xPlaySound(id, sound_nvg[1])
				}
				else
				{
					SwitchNvg(id, 0)
					xPlaySound(id, sound_nvg[0])
				}	
		}
		else set_user_gnvision(id, g_nvisionenabled[id])
	}
	
	else if(!g_isalive[id] && g_isconnected[id])
	{
		if (!g_nvg[id])
		{
			SwitchNvg(id, 1)
...
public clcmd_buyammo(id)
{
	...
	if (g_zombie[id])
	{
		zp_colored_print(id, "^x04[Zombie: The Hero]^x01 %L", id, "CMD_HUMAN_ONLY")
		return PLUGIN_HANDLED;
	}
	...
	if (g_ammopacks[id] < 1)
	{
		zp_colored_print(id, "^x04[Zombie: The Hero]^x01 %L", id, "NOT_ENOUGH_AMMO")
		return PLUGIN_HANDLED;
	}
	...
	zp_colored_print(id, "^x04[Zombie: The Hero]^x01 %L", id, "AMMO_BOUGHT")
...
public show_menu_mamet(taskid)
{
	...
	{
		post(id) // zth_supportbot.sma / amxx
	}
	
	DisplayMenu(id) // UNLOCK MENU BUY
}
...
show_menu_extras(id)
{
	...
	{
		zp_colored_print(id, "^x04[Zombie: The Hero]^x01 %L", id ,"CMD_NOT_EXTRAS")
		menu_destroy(menuid)
		return;
	}
	...
	switch (key)
	{
		case 0: // Buy Weapons
		{
			...
			if (get_pcvar_num(cvar_buycustom))
			{
				...
				WPN_AUTO_ON = 0
				zp_colored_print(id, "^x04[Zombie: The Hero]^x01 %L", id, "BUY_ENABLED")
				
				// Show menu if player hasn't yet bought anything
				if (g_canbuy[id]) DisplayMenu(id)
			}
			else
				zp_colored_print(id, "^x04[Zombie: The Hero]^x01 %L", id, "CMD_NOT")
		}
		case 1: // Extra Items
		{
			// Extra items enabled?
		...
		case 2: // Zombie Classes
		{
			...
			if (get_pcvar_num(cvar_zclasses))
				...
				zp_colored_print(id, "^x04[Zombie: The Hero]^x01 %L", id, "CMD_NOT_ZCLASSES")
		}
		case 3: // Unstuck
		{
			...
			if (g_isalive[id])
			{
				if (is_player_stuck(id))
				{
					...
					zp_colored_print(id, "^x04[Zombie: The Hero]^x01 %L", id, "CMD_NOT_STUCK")
			}
			else
				zp_colored_print(id, "^x04[Zombie: The Hero]^x01 %L", id, "CMD_NOT")
		}
		...
		case 8: // Admin Menu
		{
			...
			if (get_user_flags(id) & g_access_flag[ACCESS_ADMIN_MENU])
				...
				zp_colored_print(id, "^x04[Zombie: The Hero]^x01 %L", id, "CMD_NOT_ACCESS")
		}
	}
...
public menu_zclass(id, menuid, item)
{
	...
	zp_colored_print(id, "^x04[Zombie: The Hero]^x01 %L: %s", id, "ZOMBIE_SELECT", name)
	zp_colored_print(id, "^x04[Zombie: The Hero]^x01 %L: %d %L: %d %L: %d %L: %d%%", id, "ZOMBIE_ATTRIB1", ArrayGetCell(g_zclass_hp, g_zombieclassnext[id]), id, "ZOMBIE_ATTRIB2", ArrayGetCell(g_zclass_spd, g_zombieclassnext[id]),
...
public cmd_zombie(id, level, cid)
{
	...
	{
		client_print(id, print_console, "[Zombie: The Hero] %L", id, "CMD_NOT")
		return PLUGIN_HANDLED
	}
	...
	if (get_pcvar_num(cvar_nvggive))
	{
		...
		if (!g_isbot[id])
		{
			...
			if (get_pcvar_num(cvar_nvggive) == 1)
			{
				...
				if (get_pcvar_num(cvar_customnvg))
				{
					SwitchNvg(id, g_nvisionenabled[id])
				}
				else
					set_user_gnvision(id, 1)
			}
			...
			{
				if (get_pcvar_num(cvar_customnvg)) SwitchNvg(id, 0)
...
public welcome_msg()
{
	...
	zp_colored_print(0, "^x04[Zombie: The Hero]^x01 %L", LANG_PLAYER, "NOTICE_INFO1")
	if (!get_pcvar_num(cvar_infammo)) zp_colored_print(0, "^x04[Zombie: The Hero]^x01 %L", LANG_PLAYER, "NOTICE_INFO2")
...
check_round(leaving_player)
{
	...
	if (g_zombie[leaving_player] && fnGetZombies() == 1)
	{
		...
		zp_colored_print(0, "^x04[Zombie: The Hero]^x01 %L", LANG_PLAYER, "LAST_ZOMBIE_LEFT", g_playername[id])
	...
	else if (!g_zombie[leaving_player] && fnGetHumans() == 1)
	{
		...
		zp_colored_print(0, "^x04[Zombie: The Hero]^x01 %L", LANG_PLAYER, "LAST_HUMAN_LEFT", g_playername[id])
			...
			if (g_zombie[id])
			{
				...
				else // Zombies
				{
					...
					else
					{
						...
						//set_pev(id, pev_weaponmodel2, "")
						set_pev(id, pev_viewmodel2, infectionmodel)
						set_pev(id, pev_weaponmodel2, "")
					}
				}
			}
...
public ShowHUD(taskid)
{
	...
	else
	{
		...
		set_hudmessage(red, green, blue, -1.0, 0.90, 0, 6.0, 1.1, 0.0, 0.0, -1)
		ShowSyncHudMsg(ID_SHOWHUD, g_MsgSync2, "%L: %d", id, "ZOMBIE_ATTRIB1", pev(ID_SHOWHUD, pev_health))
	}
}
...
public native_get_user_zombie(id)
{
	if (!is_user_valid(id))
	{
		log_error(AMX_ERR_NATIVE, "[Zombie: The Hero] Invalid Player (%d)", id)
		return -1;
	}
...
public native_set_user_zombie_class(id, classid)
{
	...
	{
		log_error(AMX_ERR_NATIVE, "[Zombie: The Hero] Invalid zombie class id (%d)", classid)
		return false;
	}
...
public native_get_zombie_maxhealth(id)
{
	...
	{
		log_error(AMX_ERR_NATIVE, "[Zombie: The Hero] Player not a normal zombie (%d)", id)
		return -1;
	}
...
public native_force_buy_extra_item(id, itemid, ignorecost)
{
	...
	{
		log_error(AMX_ERR_NATIVE, "[Zombie: The Hero] Invalid extra item id (%d)", itemid)
		return false;
	}
...
public native_register_extra_item(const name[], cost, team)
{
	...
	if (!g_arrays_created)
	{
		log_error(AMX_ERR_NATIVE, "[Zombie: The Hero] Can't register extra item yet (%s)", name)
		return -1;
	}
	
	if (strlen(name) < 1)
	{
		log_error(AMX_ERR_NATIVE, "[Zombie: The Hero] Can't register extra item with an empty name")
		return -1;
	}
	...
	for (index = 0; index < g_extraitem_i; index++)
	{
		...
		{
			log_error(AMX_ERR_NATIVE, "[Zombie: The Hero] Extra item already registered (%s)", name)
			return -1;
		}
	}
...
public native_register_zombie_class(const name[], const info[], const model[], const clawmodel[], hp, speed, Float:gravity, Float:knockback)
{
	...
	if (!g_arrays_created)
	{
		log_error(AMX_ERR_NATIVE, "[Zombie: The Hero] Can't register zombie class yet (%s)", name)
		return -1;
	}
	
	if (strlen(name) < 1)
	{
		log_error(AMX_ERR_NATIVE, "[Zombie: The Hero] Can't register zombie class with an empty name")
		return -1;
	}
	...
	for (index = 0; index < g_zclass_i; index++)
	{
		...
		{
			log_error(AMX_ERR_NATIVE, "[Zombie: The Hero] Zombie class already registered (%s)", name)
			return -1;
		}
	}
...
stock xPlaySound(id, const sound[])
{
	if(id == 0)
	{
		if (equal(sound[strlen(sound)-4], ".mp3"))
