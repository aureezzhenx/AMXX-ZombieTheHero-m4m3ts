...
#define PLUGIN "[ZTH] Balrog-9"
#define VERSION "2.0"
#define AUTHOR "m4m3ts"

#define V_MODEL "models/v_balrog9_blue.mdl"
#define P_MODEL "models/p_balrog9_blue.mdl"

#define CSW_DRAGONSWORD CSW_KNIFE
#define weapon_dragonsword "weapon_knife"
#define WEAPON_ANIMEXT "dragontail" //"skullaxe"
#define KNIFE_KNOCK 3
#define DRAW_TIME 1.0

#define SLASH_ROTATE_DAMAGE 200.0
#define SLASH_ROTATE_RADIUS 70.0
#define SLASH_ROTATE_POINT_DIS 50.0
#define SLASH_ROTATE_DELAY_TIME 0.05
#define SLASH_ROTATE_RESET_TIME 0.3

#define STAB_DAMAGE 500.0
#define STAB_RADIUS 110.0
#define STAB_POINT_DIS 80.0
#define STAB_TIME 256.0
#define STAB_RESET_TIME 1.4
...
new const GRENADE_EXPLOSION[] = "sprites/flame_puff01_blue.spr"

new const DragonSword_Sound[5][] = 
{
	...
	"weapons/balrog9_wall.wav"
}

new const slash_Sound[5][] = 
{
...
new const hit_Sound[2][] = 
{
	"weapons/balrog9_hit1.wav",
	"weapons/balrog9_hit2.wav"
}

new const sExplo[] = "sprites/balrogcritical_blue.spr"
...
enum
{
	...
	DS_ANIM_IDLE3,
	DS_ANIM_DRAW,
	DS_ANIM_CHARGE_START3,
	DS_ANIM_CHARGE_START_1,
	DS_ANIM_SLASH,
	DS_ANIM_SLASH2,
	DS_ANIM_SLASH3,
	DS_ANIM_SLASH4,
	DS_ANIM_SLASH5,
	DS_ANIM_CHARGE_FINISH_1,
	DS_ANIM_CHARGE_IDLE_1,
	DS_ANIM_CHARGE_IDLE_2,
	DS_ANIM_CHARGE_ATTACK_1,
	DS_ANIM_CHARGE_ATTACK_2
}
...
new g_Old_Weapon[33], charge_ready[33], fail_shot[33], secondary_attack[33], g_Ham_Bot, g_MaxPlayers, sExplo_id, sExplohh
...
public plugin_precache()
{
	...
	for(new i = 0; i < sizeof(slash_Sound); i++)
		engfunc(EngFunc_PrecacheSound, slash_Sound[i])

	for(new i = 0; i < sizeof(hit_Sound); i++)
		engfunc(EngFunc_PrecacheSound, hit_Sound[i])

	sExplo_id = engfunc(EngFunc_PrecacheModel, sExplo)
	sExplohh = precache_model(GRENADE_EXPLOSION)
}
	
public plugin_natives()
{
	register_native("get_balrog9", "native_get_balrog9", 1)
}

public native_get_balrog9(id)
{
	get_balrog9(id)
}
...
public zp_user_infected_pre(id) remove_dragonsword(id)
public zp_user_infected_post(id) remove_dragonsword(id)
...
public get_balrog9(id)
{
	...
	if(get_user_weapon(id) == CSW_KNIFE)
	{
		...
		set_weapons_timeidle(id, CSW_DRAGONSWORD, DRAW_TIME)
...
public Do_RegisterHam_Bot(id)
{
	...
	RegisterHamFromEntity(Ham_TakeDamage, id, "TakeDamage_Post", 1)
}
...
public fw_CmdStart(id, uc_handle, seed)
{
	...
	if (CurButton & IN_ATTACK)
	{
		...
		g_Slashing_Mode[id]++
		if(g_Slashing_Mode[id] > ATTACK_SLASH_5) g_Slashing_Mode[id] = 1
		
		if(g_Slashing_Mode[id] == 1)
		{
			g_Attack_Mode[id] = ATTACK_SLASH_1
		...
		else if(g_Slashing_Mode[id] == 2)
		{
			...
			set_player_nextattack(id, SLASH_ROTATE_RESET_TIME)
			charge_ready[id] = 0
			set_weapon_anim(id, DS_ANIM_SLASH2)
			set_task(SLASH_ROTATE_DELAY_TIME, "Do_Slashing_Rotate", id+TASK_SLASHING)
		}
		else if(g_Slashing_Mode[id] == 3)
		{
			g_Attack_Mode[id] = ATTACK_SLASH_3
			g_Checking_Mode[id] = 1
			ExecuteHamB(Ham_Weapon_PrimaryAttack, ent)
			g_Checking_Mode[id] = 0
			charge_ready[id] = 0
			...
			set_weapon_anim(id, DS_ANIM_SLASH3)
			set_task(SLASH_ROTATE_DELAY_TIME, "Do_Slashing_Rotate", id+TASK_SLASHING)
		}
		else if(g_Slashing_Mode[id] == 4)
		{
			g_Attack_Mode[id] = ATTACK_SLASH_4
			g_Checking_Mode[id] = 1
			ExecuteHamB(Ham_Weapon_PrimaryAttack, ent)
			g_Checking_Mode[id] = 0
			charge_ready[id] = 0
			...
			set_weapon_anim(id, DS_ANIM_SLASH4)
			set_task(SLASH_ROTATE_DELAY_TIME, "Do_Slashing_Rotate", id+TASK_SLASHING)
		}
		else if(g_Slashing_Mode[id] == 5)
		{
			g_Attack_Mode[id] = ATTACK_SLASH_5
			g_Checking_Mode[id] = 1
			ExecuteHamB(Ham_Weapon_PrimaryAttack, ent)
			g_Checking_Mode[id] = 0
			charge_ready[id] = 0
			...
			set_weapon_anim(id, DS_ANIM_SLASH5)
			set_task(SLASH_ROTATE_DELAY_TIME, "Do_Slashing_Rotate", id+TASK_SLASHING)
		}
			...
			if(charge_ready[id])
			{
				...
				g_Checking_Mode[id] = 0
				remove_task(id+TASK_STABING)
				set_task(0.1, "Do_StabNow", id+TASK_STABING)
				set_weapon_anim(id, DS_ANIM_CHARGE_ATTACK_2)
				critical(id)
				charge_ready[id] = 0
				remove_task(id)
			}
...
public fw_secondaryattack(wpn)
{
	...
	if(g_Had_DragonSword[id])
	{
		...
		set_weapon_anim(id, DS_ANIM_CHARGE_START_1)
		set_weapons_timeidle(id, CSW_DRAGONSWORD, 0.0)
		set_player_nextattack(id, STAB_TIME)
		set_task(0.7, "charge_idle1", id)
		set_task(1.5, "check", id)
...
public check(id)
{
	if(fail_shot[id]) shuting_fail(id)
	else
	{
		charge_finish1(id)
		set_task(0.3, "charge_idle2", id)
	}
}
...
public critical(id)
{
	...
	get_position(id, 32.0, 0.0, -15.0, Origin)
	...
	write_short(sExplohh) // Sprite index
	write_byte(10) // Scale
...
public charge_idle1(id)
{
	if(get_user_weapon(id) != CSW_DRAGONSWORD || !g_Had_DragonSword[id])
		return
	set_weapon_anim(id, DS_ANIM_CHARGE_IDLE_1)
}
public charge_finish1(id)
{
	if(get_user_weapon(id) != CSW_DRAGONSWORD || !g_Had_DragonSword[id])
		return
	set_weapon_anim(id, DS_ANIM_CHARGE_FINISH_1)
	charge_ready[id] = 1
}
public charge_idle2(id)
{
	if(get_user_weapon(id) != CSW_DRAGONSWORD || !g_Had_DragonSword[id])
		return
	set_weapon_anim(id, DS_ANIM_CHARGE_IDLE_2)
}
...
public Do_Slashing_Rotate(id)
{
	...
	if(Check_Attack(id, ATTACK_SLASH_1) || Check_Attack(id, ATTACK_SLASH_2) || Check_Attack(id, ATTACK_SLASH_3) || Check_Attack(id, ATTACK_SLASH_4) || Check_Attack(id, ATTACK_SLASH_5))
	{
		emit_sound(id, CHAN_WEAPON, hit_Sound[random( sizeof(hit_Sound))], 1.0, ATTN_NORM, 0, PITCH_NORM)
	} else {
		if(g_Hit_Ing[id] == HIT_WALL) emit_sound(id, CHAN_WEAPON, DragonSword_Sound[4], 1.0, ATTN_NORM, 0, PITCH_NORM)
		else if(g_Hit_Ing[id] == HIT_NOTHING) emit_sound(id, CHAN_WEAPON, slash_Sound[random( sizeof(slash_Sound))], 1.0, ATTN_NORM, 0, PITCH_NORM)
	}
...
public Do_StabNow(id)
{
	...
	if(get_user_weapon(id) != CSW_DRAGONSWORD || !g_Had_DragonSword[id])
		return	
	set_weapons_timeidle(id, CSW_DRAGONSWORD, 0.000001)
	set_player_nextattack(id, 0.00000001)
	...
	if(Check_Attack(id, ATTACK_STAB))
	{
		...
		emit_sound(id, CHAN_WEAPON, DragonSword_Sound[3], 1.0, ATTN_NORM, 0, PITCH_NORM)
	} else {
		if(g_Hit_Ing[id] == HIT_WALL) emit_sound(id, CHAN_WEAPON, DragonSword_Sound[3], 1.0, ATTN_NORM, 0, PITCH_NORM)
		else if(g_Hit_Ing[id] == HIT_NOTHING) emit_sound(id, CHAN_WEAPON, DragonSword_Sound[3], 1.0, ATTN_NORM, 0, PITCH_NORM)
	}
...
public Check_Attack(id, Mode)
{
	...
	if(Mode == ATTACK_SLASH_1 || Mode == ATTACK_SLASH_2 || Mode == ATTACK_SLASH_3 || Mode == ATTACK_SLASH_4 || Mode == ATTACK_SLASH_5)
	{
...
public fw_TraceLine(Float:vector_start[3], Float:vector_end[3], ignored_monster, id, handle)
{
	...
	if(g_Attack_Mode[id] == ATTACK_SLASH_1 || g_Attack_Mode[id] == ATTACK_SLASH_2 || g_Attack_Mode[id] == ATTACK_SLASH_3 || g_Attack_Mode[id]== ATTACK_SLASH_4 || g_Attack_Mode[id]== ATTACK_SLASH_5) xs_vec_mul_scalar(v_forward, SLASH_ROTATE_RADIUS, v_forward)
...
public fw_TraceHull(Float:vector_start[3], Float:vector_end[3], ignored_monster, hull, id, handle)
{
	...
	if(g_Attack_Mode[id] == ATTACK_STAB) xs_vec_mul_scalar(v_forward, STAB_RADIUS, v_forward)
	else xs_vec_mul_scalar(v_forward, 0.0, v_forward)
	xs_vec_add(vecStart, v_forward, vecEnd)
...
public TakeDamage_Post(victim, inflictor, attacker, Float:damage, damagetype)
{
	...
	if(!g_Had_DragonSword[attacker] || get_user_weapon(attacker) != CSW_KNIFE || !zp_get_user_zombie(victim))
 	return HAM_IGNORED
	if(secondary_attack[attacker])
	{
...
fake_player_trace_attack(iAttacker, iVictim, &Float:fDamage)
{
	...
	switch(iHitgroup)
	{
		case HIT_HEAD: fMultifDamage  = 2.6
	...
	if(using_ds(iAttacker)) fDamage *= 3.0
	else fDamage *= fMultifDamage
