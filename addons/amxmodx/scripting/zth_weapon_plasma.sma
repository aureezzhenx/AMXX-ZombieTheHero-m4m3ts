...
public plugin_init() 
{
	...
	g_plasma = zp_register_extra_item("plasmagun", 3, ZP_TEAM_HUMAN)
}

public zp_extra_item_selected(id, itemid)
{
	// Check if the selected item matches any of our registered ones
	if (itemid == g_plasma) Get_Plasma(id)
}
...
{
    register_native("Get_Plasma", "native_Get_Plasma", 1)
}

public native_Get_Plasma(id)
{
        Get_Plasma(id)
}
