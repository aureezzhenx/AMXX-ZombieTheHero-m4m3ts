...
#define PLUGIN "[Mileage] Melee: THANATOS-9"
#define VERSION "1.0"
...
new g_HamBot, g_MaxPlayers, g_SmokePuff_SprId
...
public plugin_init()
{
	...
	//g_Thanatos9 = Mileage_RegisterWeapon("thanatos9")
}

public plugin_natives()
{
	register_native("Get_Thanatos9", "Native_Mileage_WeaponGet", 1)
}
...
public plugin_precache()
{
	...
	g_SmokePuff_SprId = precache_model(WeaponResources[0])
}
...
public Native_Mileage_WeaponGet(id)
{
	Get_Thanatos9(id)
}
...
public fw_CmdStart(id, uc_handle, seed)
{
	...
	if(Get_BitVar(g_FallenGalvatron, id))
	{
		if(get_gametime() - 0.085 > CheckDamage[id])
		{
			...
			emit_sound(id, CHAN_WEAPON, WeaponSounds[3], 1.0, ATTN_NORM, 0, PITCH_NORM)
...
public Deactivate_FallenGalvatron(id)
{
	...
	emit_sound(id, CHAN_WEAPON, WeaponSounds[4], 1.0, 0.4, 0, 94 + random_num(0, 15))
...
public Damage_Slashing(id)
{
	...
	if(Have_Victim) emit_sound(id, CHAN_STATIC, WeaponSounds[11], 1.0, 0.4, 0, 94 + random_num(0, 15))
	else {
		MyOrigin[2] += 26.0
		get_position(id, RADIUS - 5.0, 0.0, 0.0, Point[0])
		
		if(is_wall_between_points(MyOrigin, Point[0], id))
			emit_sound(id, CHAN_STATIC, WeaponSounds[12], 1.0, 0.4, 0, 94 + random_num(0, 15))
	}
}
