...
#define PLUGIN "[Mileage] Primary: JANUS-V"
#define VERSION "1.0"
...
new g_Event_Janus5, g_SmokePuff_SprId, g_RifleShell_Id, g_ham_bot
new g_janus5zp
...
public plugin_init()
{
	...
	g_janus5zp = zp_register_extra_item("Janus-5", 0, ZP_TEAM_HUMAN)
}

public zp_extra_item_selected(id, itemid)
{
	// Check if the selected item matches any of our registered ones
	if (itemid == g_janus5zp) Get_Janus5(id)
}
...
{
	register_native("Get_Janus5", "Mileage_WeaponGet", 1)
}
...
public Event_CurWeapon(id)
{
	...
	if((CSWID == CSW_JANUS5 && g_OldWeapon[id] != CSW_JANUS5) && Get_BitVar(g_Had_Janus5, id))
	{
		if(SubModel != -1) Draw_NewWeapon(id, CSWID)
	} else if((CSWID == CSW_JANUS5 && g_OldWeapon[id] == CSW_JANUS5) && Get_BitVar(g_Had_Janus5, id)) {
	...
	} else if(CSWID != CSW_JANUS5 && g_OldWeapon[id] == CSW_JANUS5) {
		if(SubModel != -1) Draw_NewWeapon(id, CSWID)
	}
...
{
	if(CSW_ID == CSW_JANUS5)
	{
		static ent
		ent = fm_get_user_weapon_entity(id, CSW_JANUS5)
		
		if(pev_valid(ent) && Get_BitVar(g_Had_Janus5, id))
		{
...
public fw_PlaybackEvent(flags, invoker, eventid, Float:delay, Float:origin[3], Float:angles[3], Float:fparam1, Float:fparam2, iParam1, iParam2, bParam1, bParam2)
{
	...
	emit_sound(invoker, CHAN_WEAPON, g_Janus5_Mode[invoker] == JANUS5_JANUS ? Janus5_Sounds[1] : Janus5_Sounds[0], 1.0, 0.4, 0, 94 + random_num(0, 15))
