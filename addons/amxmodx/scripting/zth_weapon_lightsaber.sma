...
------------ Configs -------------
=================================*/
#define DAMAGE_LIGHTSABER 140
#define DAMAGE_ATTACK 100
...
{
    register_native("Get_LightSaber", "native_Get_LightSaber", 1)
}

public native_Get_LightSaber(id)
{
        Get_LightSaber(id)
}
...
public zp_user_infected_pre(id) Remove_LightSaber(id)
...
public Event_CurWeapon(id)
{
	...
	static CSWID; CSWID = get_user_weapon(id)
	
	if(CSWID == CSW_LIGHTSABER && Get_BitVar(g_Had_LightSaber, id))
	{
		set_pev(id, pev_viewmodel2, V_MODEL)
		set_pev(id, pev_weaponmodel2, Get_BitVar(g_IsOnMode, id) ? P_MODEL_ON : P_MODEL_OFF)
...
fake_player_trace_attack(iAttacker, iVictim, &Float:fDamage)
{
	...
	switch(iHitgroup)
	{
		case HIT_HEAD: fMultifDamage  = 1.4
