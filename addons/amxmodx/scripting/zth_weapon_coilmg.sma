...
#define PLUGIN "[Mileage] Primary: Coilgun"
#define VERSION "1.0"
#define AUTHOR "Dias"

#define DAMAGE 62 // 62 for zombie
#define CLIP 100
#define BPAMMO 200
...
new g_MaxPlayers, g_MsgCurWeapon
...
public plugin_init()
{
	...
	g_coil = zp_register_extra_item("CoilGun", 0, ZP_TEAM_HUMAN)
}

public zp_extra_item_selected(id, itemid)
{
	// Check if the selected item matches any of our registered ones
	if (itemid == g_coil) Get_Coilgun(id)
}
...
{
	register_native("Get_Coilgun", "Mileage_WeaponGet", 1)
	register_native("refill_coilgun", "Mileage_WeaponRefillAmmo", 1)
}
...
public Event_CurWeapon(id)
{
	...
	if((CSWID == CSW_M249 && g_OldWeapon[id] != CSW_M249) && Get_BitVar(g_Had_Coilgun, id))
	{
		if(SubModel != -1) Draw_NewWeapon(id, CSWID)
	} else if((CSWID == CSW_M249 && g_OldWeapon[id] == CSW_M249) && Get_BitVar(g_Had_Coilgun, id)) {
		static Ent; Ent = fm_get_user_weapon_entity(id, CSW_M249)
		if(!pev_valid(Ent))
		{
	...
	} else if(CSWID != CSW_M249 && g_OldWeapon[id] == CSW_M249) {
		if(SubModel != -1) Draw_NewWeapon(id, CSWID)
	}
...
public Draw_NewWeapon(id, CSW_ID)
{
	...
	{
		static ent
		ent = fm_get_user_weapon_entity(id, CSW_M249)
		
		if(pev_valid(ent) && Get_BitVar(g_Had_Coilgun, id))
		{
