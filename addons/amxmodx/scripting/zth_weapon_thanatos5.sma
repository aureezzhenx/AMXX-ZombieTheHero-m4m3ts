...
#define PLUGIN "[Mileage] Primary: Thanatos-5"
#define VERSION "1.0"
#define AUTHOR "Dias no Pendragon"

#define DAMAGE_A 35 // 58 for zombie
#define DAMAGE_B 200 // 300 for zombie
...
#define P_MODEL "models/p_primary.mdl"
#define W_MODEL "models/w_primary.mdl"
...
new g_Thanatos5, g_OldWeapon[33]
new g_Had_Thanatos5, g_Thanatos5_Clip[33], g_GrenadeMode
new g_MsgCurWeapon, g_MsgStatusIcon, g_InTempingAttack, g_MaxPlayers
...
public plugin_init() 
{
	...
	register_touch(SCYTHE_CLASSNAME, "player", "fw_Touch")
	register_touch(SCYTHE_CLASSNAME2, "player", "fw_Touch")
	...
	g_Thanatos5 = zp_register_extra_item("Thanatos-5", 0, ZP_TEAM_HUMAN)
}

public zp_extra_item_selected(id, itemid)
{
	// Check if the selected item matches any of our registered ones
	if (itemid == g_Thanatos5) Get_Thanatos5(id)
}
...
public Event_CurWeapon(id)
{
	...
	if((CSWID == CSW_THANATOS5 && g_OldWeapon[id] != CSW_THANATOS5) && Get_BitVar(g_Had_Thanatos5, id))
	{
		if(SubModel != -1) Draw_NewWeapon(id, CSWID)
	} else if((CSWID == CSW_THANATOS5 && g_OldWeapon[id] == CSW_THANATOS5) && Get_BitVar(g_Had_Thanatos5, id)) {
		
	} else if(CSWID != CSW_THANATOS5 && g_OldWeapon[id] == CSW_THANATOS5) {
		if(SubModel != -1) Draw_NewWeapon(id, CSWID)
	}
...
{
	if(CSW_ID == CSW_THANATOS5)
	{
		static ent
		ent = fm_get_user_weapon_entity(id, CSW_THANATOS5)
		
		if(pev_valid(ent) && Get_BitVar(g_Had_Thanatos5, id))
		{
...
{
	register_native("Get_Thanatos5", "Mileage_WeaponGet", 1)
}

public Mileage_WeaponGet(id)
{
	Get_Thanatos5(id)
}

public Mileage_WeaponRefillAmmo(id)
{
	cs_set_user_bpammo(id, CSW_THANATOS5, 200)
}

public Mileage_WeaponRemove(id)
{
	Remove_Thanatos5(id)
}
...
{
	set_task(random_float(0.1, 0.5), "Reset_Now", id)	
...
public Reset_Now(id)
{
	if(is_user_alive(id)) Update_SpecialAmmo(id, 1, 0)
}
